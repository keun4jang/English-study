...
**현재 버전: v0.3.x** · 외부 서비스 없이 기기 안에서 완전히 동작합니다 (영구 무료)
...
- **AI**: 내장 규칙 엔진(`src/ai/rules/`) — 외부 API 호출 없음. Provider Adapter로 교체 가능(선택)
...
- 친구 공유/댓글은 서버가 필요해 활성화하지 않았습니다 (DB 스키마·RLS만 준비됨)
