...
export const typography: Record<string, TypeToken> = {
  ...
  /**
   * 오늘의 편지 질문 — 앱이 말을 거는 한 문장.
   *
   * 한때 손글씨 폰트(Gaegu)를 썼는데 읽기가 불편해서 본문 폰트로 되돌렸다. 감성을 위해
   * 매일 읽는 문장의 가독성을 깎는 건 남는 장사가 아니다. 대신 크기와 굵기로만 구분한다 —
   * heading(22/700)만큼 큰 소리는 아니고 body보다는 분명한, 질문에 맞는 톤.
   */
  prompt: { fontSize: 21, lineHeight: 32, fontWeight: '600', fontFamily: fonts.ui, letterSpacing: -0.2 },
