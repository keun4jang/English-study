...
export interface KoParse {
   ...
   * **알아봤는데 쓰지 못한 말.**
   *
   * 자리(장소·목적어)가 이미 차 있으면 그 다음 단어는 갈 곳이 없다. 예전에는 조용히
   * 버리고 성공한 척했는데, 그래서 "오늘 여의도 한강 공원을 뛰면서 영상과 사진 촬영을 했어"가
   * "I had a shoot in [뛰면서] today."가 됐다 — 한강·공원·영상·사진을 전부 알아보고도 버린 것이다.
   *
   * 여기에 뭔가 담겼다는 건 이 문장이 우리 모델보다 복잡하다는 뜻이다. 그러면 만들지 않는다.
   */
  dropped: string[];
  /**
   * 우리가 다루지 못하는 **다른 서술어**로 보이는 말 (예: '뛰면서').
   *
   * 연결어미가 붙어 있으면 절이 하나 더 있다는 뜻이다. 지금 엔진은 절 하나만 다루므로,
   * 이게 있으면 문장의 절반을 못 읽고 있는 것이다.
   */
  unhandled: string[];
}
...
export interface KoHelp {
  ...
  /**
   * 문장을 끝까지 읽지 못했을 때 그 이유.
   *
   * 'too-complex' — 절이 여럿이거나 담을 자리보다 내용이 많다
   * 'too-little'  — 아는 말이 거의 없다
   * null          — 이유가 딱히 없다(그냥 사전에 없는 문형)
   */
  cannotReason: 'too-complex' | 'too-little' | null;
  /** 알아본 말들 — 되묻기에 쓴다 */
  understood: string[];
}
